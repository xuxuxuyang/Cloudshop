...
const {postcode} = require('../../config/checking.js')// 校验
const {Auth} = require('../../token/auth.js')// 验证token合法性
const {buffer} = require('../../cos/cos')// 图片上传
const moment = require('moment')// 时间模块
moment.locale('zh-cn')
const {Code} = require('../../config/code-img.js')// 给二进制图片重新命名

// 添加桌号
router.post('/qrcode', new Auth().m, async ctx=>{
	let {table} = ctx.request.body
	new postcode(ctx,table).start()// 校验
	let time = moment().utcOffset(8).format('YYYY-MM-DD HH:mm:ss')
	try{
		const query = `db.collection('table_qr_code').where({table:'${table}'}).get()`	// 查询该桌号是否已存在
