...
const {getToken,Addurl,Tripurl,Updateurl} = require('../../config/databaseapi.js')// 操作数据库的接口
const {shopinfor,catecheck,unitcheck,putoncheck} = require('../../config/checking.js')// 校验
const {Auth} = require('../../token/auth.js')// 验证token合法性
const {upload,cosfun} = require('../../cos/cos')// 图片上传
...
router.get('/checkout', new Auth().m, async ctx=>{  // 结账以及推送订阅消息new Auth().m,
	let {id,openid,sett_amount,order_no} = ctx.query
	let newmoney = Price(Number(sett_amount))
	let time = moment().utcOffset(8).format('YYYY-MM-DD HH:mm:ss')// 订阅消息字段组合
